...
class VGG11UNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        d5 = self.dec5(torch.cat((d5, features['relu5']), dim=1))
        
        d4 = self.upconv4(d5)
        d4 = self.dec4(torch.cat((d4, features['relu4']), dim=1))
        
        d3 = self.upconv3(d4)
        d3 = self.dec3(torch.cat((d3, features['relu3']), dim=1))
        
        d2 = self.upconv2(d3)
        d2 = self.dec2(torch.cat((d2, features['relu2']), dim=1))
